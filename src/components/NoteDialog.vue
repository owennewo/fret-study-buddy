...
<template>
  <div class="field">
    ...
    <div v-if="bar" class="field-group">
      <p-inputnumber v-model="bar.timeSignature.beatsPerBar" placeholder="Beats per Bar" :min="1" showButtons
        class="small" />
      <span>&nbsp;/&nbsp;</span>
      <p-inputnumber v-model="bar.timeSignature.beatValue" placeholder="Beat Value" :min="1" showButtons
        class="small" />
    </div>
  </div>
  <h2>Element</h2>
  ...
  <p-floatlabel variant="on">
    <p-select v-model="element.duration.beats" :options="beatsOptions" optionLabel="label" optionValue="value"
      inputId="baseUnit" placeholder="Base Unit" class="w-full">
    </p-select>

    <label for="duration">Duration</label>
  </p-floatlabel>
  <p-floatlabel variant="on">
    <p-inputnumber v-model="element.duration.dotCount" inputId="dotCount" variant="filled" :min="0" :max="2" :step="1"
      showButtons />
  ...
  <p-floatlabel variant="on">
    <p-inputtext inputId="position" :value="`${element.location()} - ${element.location() + element.beatDuration()}`"
  ...
  <h2>Note</h2>
  <p-inputnumber v-model="note.fret" placeholder="fret" showButtons class="small" />
  ...
  <p-multiselect v-model="note.techniques" :options="techniqueList()" optionLabel="name" optionValue="key"
    inputId="techniqueSelect" placeholder="Select Techniques" display="chip" :showToggleAll="false" class="w-full">
  </p-multiselect>
</template>
